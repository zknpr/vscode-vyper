...
function createHover(name, snippet, _type) {
    function isSet(val) {
    ...
    if (isSet(snippet.description)) {
        const txt_descr = snippet.description instanceof Array ? snippet.description.join('\n* ') : snippet.description;
        text.push('💡 ' + txt_descr);
    }
    ...
    if (isSet(snippet.security)) {
        text.push('');
        const txt_security = snippet.security instanceof Array ? snippet.security.join('\n* ❗') : snippet.security;
...
function provideHoverHandler(document, position, token, type) {
    ...
    const range = document.getWordRangeAtPosition(position, /(tx\.gasprice|tx\.origin|msg\.data|msg\.sender|msg\.sig|msg\.value|block\.coinbase|block\.difficulty|block\.gaslimit|block\.number|block\.timestamp|abi\.encodePacked|abi\.encodeWithSelector|abi\.encodeWithSignature|abi\.decode|abi\.encode|\.?[\w>]+)/);
